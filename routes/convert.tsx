...
export const handler: Handlers<Data> = {
  async GET(req, ctx) {
    ...
    const csResponse = await fetch(
      `https://api.currencyscoop.com/v1/convert?api_key=${
        Deno.env.get("API_KEY")
      }&from=${from}&to=${to}&amount=${amount}`,
    );
    const csResult = await csResponse.json();
    return ctx.render({
      convertedAmount: csResult.response.value,
      amount,
      from,
      to,
    });
    ...
    <main className="flex justify-center items-center h-screen">
      <div className="p-4 max-w-screen-md bg-white shadow-md rounded px-8 pt-6 pb-6">
        ...
        <div className="font-medium">
          <span className="text-slate-200">{data.amount} {data.from} =</span>
          <br />
          <span className="text-2xl">
            {data.convertedAmount.toFixed(2)} {data.to}
          </span>
        </div>
      </div>
    </main>
